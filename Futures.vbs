...
Const cstrFilename  = "fo{DD}{MMM}{YYYY}bhav.csv"

Const cstrBHAVCOPYURL  = "http://www.nseindia.com/content/historical/DERIVATIVES/{YYYY}/{MMM}/"
...
Dim gdtmDateFrom, gdtmDateTo, gstrProxyServer, gstrProxyUsername, gstrProxyPassword, gDBPath
...
Sub Main()
  ...
  For i = 0 to intNumDays
    ...
    'WScript.Echo "URL to process: " & strBHAVCOPYurl

    Call ProcessFile(strBHAVCOPYurl, dtmDate)
  Next
...
'****************************************************
'Sub processes the url for given day
'****************************************************
Sub ProcessFile(strURL, dtmDate)
  'Get HTML
  Dim strHTML, astrRows, strRow, astrFields, dicEquity, objStock, objQuote, strStockTicker, strStockName, strOutputText, lngResult
  
  'strURL = "C:\Mandar\Scripting\NSEData\fo15OCT2009bhav.csv"
  'strHTML = ReadFromFile(strURL)
  strHTML = ReadFileFromWeb(strURL)
  

  astrRows = Split(strHTML, vbLf)
  If UBound(astrRows) < 10 Then Exit Sub

  Set dicEquity = CreateObject("Scripting.Dictionary")
  
  strOutputText = "<ticker>,<name>,<date>,<open>,<high>,<low>,<close>,<volume>,<o/i>,(C)" & vbCrLf
  
  For each strRow in astrRows
    If Trim(strRow) <> "" Then
      astrFields = Split(strRow,",")
      
      If UBound(astrFields) > 5 Then
        If astrFields(0) = "FUTIDX" Or astrFields(0) = "FUTSTK" Then
      
          strStockTicker = astrFields(1) + "-I"
          If dicEquity.Exists(strStockTicker) Then 
            strStockTicker = astrFields(1) + "-II"
            If dicEquity.Exists(strStockTicker) Then strStockTicker = astrFields(1) + "-III"
          End If

          strStockName =  strStockTicker & " FUTURES  " & astrFields(2)
          'WScript.Echo astrFields(5) & vbTab & strRow
          dicEquity.Add strStockTicker, strRow
          
          'OPEN,HIGH,LOW,CLOSE,SETTLE_PR,CONTRACTS
          '    5       6       7       8           9                10
          Set objStock = AmiBroker.Stocks.Add(strStockTicker)
          objStock.FullName = strStockName
          objStock.GroupID = 253
          
          'Set objQuote = objStock.Quotations.Add(CDate(astrFields(14)))

          'objQuote.High = astrFields(5)
          'objQuote.Low = astrFields(6)
          'objQuote.Open = astrFields(7)
          'objQuote.Close = astrFields(8)
	
          'objQuote.Volume = astrFields(10)
          
          strOutputText = strOutputText & strStockTicker & "," & strStockName & "," & astrFields(14) & "," & astrFields(5) & "," & astrFields(6) & "," & astrFields(7) & "," & astrFields(8) & "," & astrFields(10) & "," & astrFields(12) & "," & vbCrLf
        End If
      End If
    End If
  Next
  
  Call WriteToFile(GetTempFolderPath() & "\" & GetFilenameForDate(dtmDate), strOutputText)
  
	WScript.Echo "Importing file=" & GetFilenameForDate(dtmDate)
  'WScript.Echo "Importing file=" & GetTempFolderPath() & "\Futures.format"
	
	lngResult = AmiBroker.Import(0,GetTempFolderPath() & "\" & GetFilenameForDate(dtmDate),GetTempFolderPath() & "\Futures.format")
...
Sub Usage(strMessage)
  ...
  const L_Echo14_Text = "  -DBPath	- Specify Database path for futures"
...
Sub InitialiseGlobals()
  ...
  gstrProxyServer = "":gstrProxyUsername = "":gstrProxyPassword = "":gDBPath = ""
  Dim i 
  i = 0
  Do While i < WScript.Arguments.Count
    ...
    Elseif WScript.Arguments(i) = L_Refresh Then
		...
		ElseIf WScript.Arguments(i) = L_DateDelta Then
			...
			gdtmDateTo = Date
		ElseIf WScript.Arguments(i) = L_DBPath Then
			i = i + 1
			If i >= WScript.Arguments.Count Then
				Call Usage("")
			End If
			gDBPath = WScript.Arguments(i)
    End If
    i = i + 1
  Loop
  
  Call Amibroker.LoadDatabase(gDBPath)
  If Amibroker.DatabasePath = "" Then
    Call Usage("Please enter a valid database path for futures.")
  End If
...
Sub DoIndex(objStock)
  Dim strCsvURL, strCsvData, astrRows, astrFields, dtmDate, i, j, objQuote

  WScript.Echo "Processing Index: " & objStock.Ticker
  strCsvURL = GetCsvURL(objStock.Ticker)
  
  'WScript.Echo "Index=" & objStock.Ticker & " Url=" & strCsvURL
  ...
  'WScript.Echo ""
  'WScript.Echo ""
    'WScript.Echo "Processing Row=" & astrRows(i)
...
Function GetCsvURL(strIdxTicker)
  ...
  strFormURL = "http://www.nseindia.com/marketinfo/indices/histdata/historicalindices.jsp?FromDate={dd}-{mm}-{yyyy}&IndexType={TICKER}&Indicesdata=Get%20Details&ToDate={ddT}-{mmT}-{yyyyT}&check=new"
  ...
  'WScript.Echo "GetCsvURL: IdxUrl=" & strFormURL
